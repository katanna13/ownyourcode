...
class LearningPathService:
    ...
    def _module_response(self, session: Session, module: ProjectLearningModule) -> LearningPathModuleResponse:
        definition = self._definition(module)
        progress = self._module_progress(session, module.id)
        adaptations = list(session.scalars(select(ProjectModuleAdaptation).where(ProjectModuleAdaptation.module_id == module.id, ProjectModuleAdaptation.state == ModuleAdaptationState.AVAILABLE)))
    ...
    def _activity(self, session: Session, module: ProjectLearningModule, activity_id: str) -> tuple[StoredActivityDefinition, ProjectModuleAdaptation | None]:
        ...
        adaptation = session.scalar(select(ProjectModuleAdaptation).where(ProjectModuleAdaptation.module_id == module.id, ProjectModuleAdaptation.activity_id == activity_id, ProjectModuleAdaptation.state == ModuleAdaptationState.AVAILABLE))
